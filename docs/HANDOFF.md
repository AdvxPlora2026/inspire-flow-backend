...
品牌 owner 和 member 都可以基于请求内 `project_brief` 生成即时 AI 投顾报告，并可选
引用当前用户自己的项目补充上下文。报告包含真实网页证据、项目关联、推导逻辑、行动、
风险、反方观点和置信度；弱证据不能产生高置信度结论。接口不保存报告历史，完整请求、
响应字段、证据门槛和未来品牌共享项目升级方向见
[品牌项目 AI 投顾接口接入说明](HANDOFF_ADVISORY.md)。
...
| 列出当前品牌 | 只返回当前用户具有成员权限的品牌 |
| 分析品牌项目 | 调用受限网页研究并返回带证据、推导、风险和置信度的结构化报告 |
